...
MODEL_EXPORT_DIR = project_folder # will be updated in main
...
def build_hub_module(model, num_classes, global_step, checkpoint_path):
  ...
  def module_fn_simclr_bit(is_training):
    ...
    def bit_generator(depth, width_multiplier):
      ...
      pre_logits, endpoints = resnet.bit.bit_embedding(
          inputs, model_name=model_name, trainable=is_training
      )
  ...
  hub_export_dir = os.path.join(MODEL_EXPORT_DIR, 'hub')
...
def perform_evaluation(
  ...
  result_json_path = os.path.join(MODEL_EXPORT_DIR, 'result.json')
  with tf.io.gfile.GFile(result_json_path, 'w') as f:
    json.dump({k: float(v) for k, v in result.items()}, f)
  result_json_path = os.path.join(
      MODEL_EXPORT_DIR, 'result_%d.json' % result['global_step']
  )
  with tf.io.gfile.GFile(result_json_path, 'w') as f:
    json.dump({k: float(v) for k, v in result.items()}, f)
  flag_json_path = os.path.join(MODEL_EXPORT_DIR, 'flags.json')
...
def main(argv):
  ...
  num_classes = builder.info()['num_classes']
  ...
  if _VERBOSE.value:
    #------------------- PRINT CONFIGURATION -------------------#
    tf.logging.info("Start: %s\n", START_TIME)
    tf.logging.info('GPU: %s', str(tf.config.list_physical_devices('GPU')))

    tf.logging.info('\n===== Stats =====')
    tf.logging.info('Dataset name: %s', _DATASET.value)
    tf.logging.info('Batch size: %d', _TRAIN_BATCH_SIZE.value)
    tf.logging.info('Image size: %d', _IMAGE_SIZE.value)
  ...
  if _MODE.value == 'eval':
    ...
    # ckpt_path = os.path.join(project_folder, '../out/logs', 'hub')
