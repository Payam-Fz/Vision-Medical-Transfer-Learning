...
# from tensorflow_estimator import estimator as tf_estimator  # pylint: disable=g-deprecated-tf-checker
...
def build_model_fn(model, num_classes, num_train_examples, logdir, checkpoint_dir):
  ...
  def model_fn(features, labels, mode, params=None):
    ...
    if is_training:
      if FLAGS.train_summary_steps > 0:
        ...
        summary_writer = tf2.summary.create_file_writer(logdir)
      ...
      if FLAGS.checkpoint:
        ...
        if FLAGS.rescale_at_init != 1.0 and not FLAGS.zero_init_logits_layer:
          ...
          @tf.function
          def scaffold_fn():
            # visualize checkpoint variables
            # print('__________checkpoint variables')
            # print(f"Variable: {v}" for v in tf.train.list_variables(checkpoint_dir))
            # print('______________global variables')
            # print(f"Variable: {v.name}, Op Name: {v.op.name}" for v in tf.global_variables(FLAGS.variable_schema))
            # checkpoint_var_names = tf.train.list_variables(checkpoint_dir)
            # model_var_names = tf.global_variables(FLAGS.variable_schema)
            # variable_mapping = {}
            # for checkpoint_vars, model_vars in zip(checkpoint_var_names, model_var_names):
            #   variable_mapping[tuple(checkpoint_vars)] = tuple(model_vars)
            """Scaffold function to restore non-logits vars from checkpoint."""
            tf.train.init_from_checkpoint(
                checkpoint_dir,
                {
                ...
                # {
                #   tuple(checkpoint_var_names): tuple(model_var_names)
                # },
                # variable_mapping
            )
