...
from utils.objective_func import soft_f1_loss, macro_f1_score, global_accuracy, global_precision, global_recall
...
class BaseModel(keras.Model):
    ...
    def train_step(self, data):
        ...
        with self.writer.as_default(step=self._train_counter):
            ...
            display_image = to_channels_last(x, batched=True)
...
def to_channels_last(img, batched, label=None):
    if batched:
        order = [0, 2, 3, 1]
    else:
        order = [1, 2, 0]
...
def to_channels_first(img, batched, label=None):
    if batched:
        order = [0, 3, 1, 2]
    else:
        order = [2, 0, 1]
...
def main(argv):
    ...
    # inputs of google/vit is expected to be channel-first, [0.0 - 1.0]
    ...
    def _match_model_input(x, y, info=None, batched=False):
        ...
        x = tf.image.convert_image_dtype(x, dtype=tf.float32) # scales image back to 0.0 - 1.0 # in range
        x = to_channels_first(x, batched)
        if (batched):
            x = tf.ensure_shape(x, [None,3, *IMAGE_SIZE])
    ...
    for num_unfrozen_blocks in round_range:
        ...
        earlystopping_callback = keras.callbacks.EarlyStopping(
            ...
            mode='min',
            verbose=1,
            # min_delta=0.0001,   # how much change is considered an improvement
